...
const Pagination = ({queries}) => {
    ...
    return <div className="flex flex-col items-center py-14 max-w-[700px]">
        <div className="relative text-[#4285f4]">
            {queries.previousPage && (
                <div className="absolute left-[-30px] md:left-[-40px] top-[10px]" onClick={()=>paginationClickHandle(queries.previousPage[0].startIndex)}>
                    ...
                    <div className="cursor-pointer absolute left-[-5px] top-5 hidden md:block">
                        Prev
                    </div>
                </div>
            )}
        ...
        <div className="flex gap-3 text-[#4285f4] text-sm">
            {pagination.map((p)=>(
                <span key={p.page} className={`cursor-pointer ${page === p.startIndex ? "text-black" : ""}`} onClick={()=>paginationClickHandle(p.startIndex)}>
                    {p.page}
                </span>
            ))}
        </div>
